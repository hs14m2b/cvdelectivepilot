...
def lambda_handler(event, context):
    ...
    with open('/tmp/'+file_name) as csvDataFile:
        ...
        for row in csvReader:
            ...
            if row[1] != "Processed":
                logger.debug(f"encountered an error processing row {row_count}, {row[1]}")
                errors.append({"mobile": row[0], "error_message": row[1]})
                failed_count += 1
            else:
                success_count += 1
    ...
    sns.publish(
        ...
        Subject="Covid-19 Elective Pilot File Uploaded"
    )
